...
describe ProductLocation do
  ...
  describe 'validations' do
    ...
    it 'should be invalid if warehouse is not specified' do
      expect(ProductLocation.new(product_id: product.id, quantity: 1)).to be_invalid
    end

    it 'should be invalid if quantity less than zero' do
      expect(
        ProductLocation.new(product_id: product.id, warehouse_id: main_warehouse.id, quantity: -1)
      ).to be_invalid
    end
